/**
 * planos — /planos-prd graceful interruption tests (plain Node, zero dependencies).
 *
 * Covers AC-P6 (degradation path):
 ...
 *
 * Regression guard: re-runs all planos-prd-command.test.mjs assertions in-process
 * to confirm the graceful-interruption additions did not break any prior passing test.
 *
 * Run: node tests/planos-prd-interrupt.test.mjs
 */
...
test("command's interruption path still reaches the server boot step (bin/planos prd)", () => {
  const src = readCommand();
  // bin/planos prd must appear after the interruption section
  const interruptIdx = src.search(/graceful interruption|early.exit/i);
  const bootIdx = src.lastIndexOf("bin/planos prd");
  assert.ok(interruptIdx !== -1, "No interruption section found");
  assert.ok(bootIdx !== -1, "bin/planos prd invocation not found in command");
  assert.ok(
    bootIdx > interruptIdx,
    "bin/planos prd invocation must appear after the interruption section (loop always reachable)"
  );
});
...
// ---------------------------------------------------------------------------
// 5. Minimal valid v2 PRD doc on interruption path
// ---------------------------------------------------------------------------

console.log("\n── Minimal valid v2 doc on interruption path ──");

test("command's interruption fallback produces a valid JSON block document", () => {
  const src = readCommand();
  // Fallback must still produce a JSON block document (not refuse)
  const hasFallbackDoc =
    src.includes("minimal document") ||
    src.includes("minimal valid") ||
    src.includes("emit a minimal") ||
    src.includes("minimal v2") ||
    src.includes("valid JSON block document");
  assert.ok(
    hasFallbackDoc,
    "Command must instruct emitting a minimal valid JSON block document on interruption"
  );
});

test("command's interruption fallback uses openQuestion for unknown goal", () => {
  const src = readCommand();
  // The fallback must mention using openQuestion blocks when intent is unclear
  const hasOpenQuestion =
    src.includes("openQuestion") &&
    (src.includes("recovery") || src.includes("minimal") || src.includes("interrupted"));
  assert.ok(
    hasOpenQuestion,
    "Command must instruct using openQuestion blocks as recovery when the goal is unclear after interruption"
  );
});

// ---------------------------------------------------------------------------
// 6. Regression guard — re-run planos-prd-command.test.mjs
// ---------------------------------------------------------------------------

console.log("\n── Regression guard (re-run planos-prd-command.test.mjs) ──");

test("planos-prd-command.test.mjs still passes with zero failures", () => {
  const result = spawnSync(
    process.execPath,
    ["--test", "tests/planos-prd-command.test.mjs"],
    { cwd: ROOT, encoding: "utf8", timeout: 15000 }
  );
  assert.equal(
    result.status,
    0,
    `planos-prd-command.test.mjs reported failures:\n${result.stdout}\n${result.stderr}`
  );
});
...
console.log("\n  MANUAL SMOKE (AC-P6 [M]) — document only, not automated:");
console.log("  Start /planos-prd, interrupt mid-interview with 'just build it',");
console.log("  confirm: agent stops questions, states reduced clarity, proceeds to");
console.log("  Phase 2 authoring, pipes JSON into node bin/planos prd, browser opens.");
console.log("  No crash. See docs/notes/planos-prd-command.md Scenario C for full steps.");
